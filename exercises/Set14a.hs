...
import Data.Word
import qualified Language.Haskell.TH.Ppr as T
import Mooc.Todo
...
-- shout s =
--   let words = T.words s
--    in T.unwords $ map (\(s, idx) -> if idx `mod` 2 == 0 then s else T.toUpper s) (zip words [1 ..])
shout :: T.Text -> T.Text
shout s =
  let words = T.words s
   in T.unwords $ zipWith ($) (cycle [T.toUpper, id]) words
...
longestRepeat s = case T.uncons s of
  Nothing -> 0
  Just (x, xs) ->
    let suffix = T.dropWhile (== x) xs
     in let rhs = longestRepeat suffix
         in max (T.length xs - T.length suffix + 1) rhs
...
xorChecksum s = if B.length s == 0 then 0 else B.foldr1 xor s
...
pingpong s = BL.cycle $ BL.concat [BL.fromStrict s, BL.fromStrict (B.reverse s)]
