...
totalPrice (MkShoppingEntry itemName price count) = price * fromIntegral count
...
buyOneMore (MkShoppingEntry itemName price count) = MkShoppingEntry itemName price (count + 1)
...
tick :: UpDown -> UpDown
tick (Increasing t) = Increasing (t + 1)
tick (Decreasing t) = Decreasing (t - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (Increasing t) = Decreasing t
toggle (Decreasing t) = Increasing t
...
fromNat :: Nat -> Int
fromNat Zero = 0
fromNat (PlusOne r) = 1 + fromNat r

toNat :: Int -> Maybe Nat
toNat x
  | x < 0 = Nothing
  | x == 0 = Just Zero
  | x > 0 = case toNat (x - 1) of
      Nothing -> Nothing
      Just c -> Just $ PlusOne c
...
prettyPrint (I i) = prettyPrint i ++ "1"

fromBin :: Bin -> Int
fromBin End = 0
fromBin (O o) = 0 + 2 * fromBin o
fromBin (I i) = 1 + 2 * fromBin i
