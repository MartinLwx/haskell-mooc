...
add v (Set (x : xs))
  | v < x = Set (v : x : xs)
  | v == x = Set (x : xs)
  | otherwise = let (Set ans) = add v (Set xs) in Set (x : ans)
...
data State = Start | Error | Finished | AfterAddEgg | AfterAddFlour | AfterAddSugar | AFterAddFlourAndSugar | AfterMix
  deriving (Eq, Show)

step :: State -> Event -> State
step Start AddEggs = AfterAddEgg
step AfterAddEgg AddFlour = AfterAddFlour
step AfterAddEgg AddSugar = AfterAddSugar
step AfterAddFlour AddSugar = AFterAddFlourAndSugar
step AfterAddSugar AddFlour = AFterAddFlourAndSugar
step AFterAddFlourAndSugar Mix = AfterMix
...
reverseNonEmpty' [] acc = acc
reverseNonEmpty' (x : xs) acc = reverseNonEmpty' xs (x :| toList acc)
...
instance (Ord a) => Semigroup (Set a) where
  (Set []) <> x = x
  x <> (Set []) = x
  (Set (x : xs)) <> (Set (y : ys)) = Set xs <> add x (Set (y : ys))
...
passwordAllowed s (ContainsSome charSet) = any (\ch -> isInfixOf [ch] s) charSet
passwordAllowed s (DoesNotContain charSet) = all (\ch -> not (isInfixOf [ch] s)) charSet
passwordAllowed s (And t1 t2) = passwordAllowed s t1 && passwordAllowed s t2
passwordAllowed s (Or t1 t2) = passwordAllowed s t1 || passwordAllowed s t2
...
evaluate (Multiply t1 t2) = (*) (evaluate t1) (evaluate t2)

render :: Arithmetic -> String
render (Literal x) = show x
render (Add t1 t2) = "(" ++ render t1 ++ "+" ++ render t2 ++ ")"
render (Multiply t1 t2) = "(" ++ render t1 ++ "*" ++ render t2 ++ ")"
