  ...
  if place `elem` visited
    then return ()
      ...
      case lookup place maze of
        Nothing -> return ()
        Just neighbors -> do
          forM_ neighbors $ (\c -> do visit maze c)
...
allSums :: [Int] -> [Int]
allSums xs = sum <$> filterM (const [True, False]) xs
...
instance Functor SL where
  ...
  fmap f (SL sl) = SL h
    where
      h x = let (v, s, msg) = sl x in (f v, s, msg)
...
instance Monad SL where
  ...
  f >>= g = SL h
    where
      h x =
        let (v1, s1, msg) = runSL f x
         in let (v2, s2, newlog) = runSL (g v1) s1
             in (v2, s2, msg ++ newlog)
...
mkCounter = do
  v <- newIORef 0
  return $ (modifyIORef v (+ 1), readIORef v)
