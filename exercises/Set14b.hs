...
parseCommand (x : xs)
  | x == T.pack "deposit" && length xs == 2 = case parseInt (last xs) of
      Nothing -> Nothing
      Just v -> Just $ Deposit (head xs) v
  | x == T.pack "withdraw" && length xs == 2 = case parseInt (last xs) of
      Nothing -> Nothing
      Just v -> Just $ Deposit (head xs) (-v)
  | x == T.pack "balance" && length xs == 1 = Just $ Balance (head xs)
  | otherwise = Nothing

-- then do
--   v <- parseInt (last xs)
--   let v1 = if x == T.pack "deposit" then v else -v
-- else
--   if x == T.pack "balance"
--     then Just $ Balance (head xs)
--     else Nothing
...
server db request respond = do
  let args = pathInfo request
  res <- perform db (parseCommand args)
