...
sums :: Int -> [Int]
sums i = sums' ((1 + i) * i `div` 2) i []

-- Initialy, the val is equal to the sum of 1+2+..+i
sums' val 0 res = res
sums' val i [] = sums' (val - i) (i - 1) [val]
sums' val i (x : xs) = sums' (val - i) (i - 1) (val : x : xs)
...
indexDefault xs i def = indexDefault' xs 0 i def

indexDefault' [] cur i def = def
indexDefault' (x : xs) cur i def
  | cur == i = x
  | otherwise = indexDefault' xs (cur + 1) i def
...
sumsOf' val [] = []
sumsOf' val (x : xs) = (val + x) : sumsOf' (val + x) xs
...
mymaximum bigger initial (x : xs) =
  let otherMax = mymaximum bigger initial xs
   in if bigger x otherMax
        then x
        else otherMax
