  ...
  do
    if n == 0
      then return []
        ...
        line <- getLine
        others <- readWords (n - 1)
        return $ sort (line : others)
...
countdownPrint n = countdownPrint' n 0

countdownPrint' n cur = do
  if cur == n
    then putStrLn "0"
    else do
      putStrLn (show (n - cur))
      countdownPrint' n (cur + 1)
...
isums' n curSum = do
  line <- getLine
  let val = read line
  putStrLn $ show (curSum + val)
  isums' (n - 1) (curSum + val)
