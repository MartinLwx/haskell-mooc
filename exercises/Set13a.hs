...
split s =
  let (lhs, rhs) = break (\ch -> ch == ' ') s
   in if length rhs == 0
        then Nothing
        else Just (lhs, tail rhs) -- tail rhs to remove whitespace
...
checkNumber (s1, s2) = if any isDigit s1 || any isDigit s2 then Nothing else Just (s1, s2)
...
checkCapitals (s1, s2) = if (isAsciiUpper . head) s1 && (isAsciiUpper . head) s2 then Just (s1, s2) else Nothing
...
balance accountName = BankOp f
  where
    f (Bank t) = (Map.findWithDefault 0 accountName t, Bank t)
...
rob from to = balance from +> \val -> withdrawOp from val +> \val -> depositOp to val
...
paren '(' = do
  curState <- get
  put $ if curState == -1 then -1 else curState + 1
paren ')' = do
  curState <- get
  put $ if curState == -1 then -1 else curState - 1
...
count x = do
  curState <- get
  put $ helper curState x

-- helper :: (Eq a) => State [(a, Int)]
helper [] key = [(key, 1)]
helper ((k, v) : xs) key
  | k == key = (k, v + 1) : xs
  | otherwise = (k, v) : helper xs key
...
occurrences xs = do
  curState <- get
  let newState = foldr (\c acc -> helper acc c) curState xs
  put newState
