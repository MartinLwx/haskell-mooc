...
count v xs = foldr (\c acc -> if c == v then 1 + acc else acc) 0 xs
...
inBoth a b =
  let lhs = toList a
      rhs = toList b
   in filter (\x -> x `elem` lhs && x `elem` rhs) lhs
...
instance Foldable List where
  ...
  foldr f acc (LNode val rest) = f val (foldr f acc rest)
