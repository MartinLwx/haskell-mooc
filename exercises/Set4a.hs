...
rangeOf x = let y = sort x in last y - head y
...
longest [x] = x
longest (x : xs) =
  let otherLongest = longest xs
   in if length x > length otherLongest
        || length x == length otherLongest
          && head x < head otherLongest
        then x
        else otherLongest
...
incrementKey target [] = []
incrementKey target ((k, v) : xs)
  | target == k = (k, v + 1) : incrementKey target xs
  | otherwise = (k, v) : incrementKey target xs
...
freqs xs = foldr (\c acc -> Map.alter addOne c acc) (Map.fromList []) xs
...
transfer from to amount bank
  ...
  | otherwise =
      let Just fromAmount = Map.lookup from bank
          Just toAmount = Map.lookup to bank
       in if fromAmount < amount
            then bank
            else Map.insert to (toAmount + amount) (Map.insert from (fromAmount - amount) bank)
...
maxIndex' [(k, v)] = (k, v)
maxIndex' ((k, v) : xs) = let (k1, v1) = maxIndex' xs in if v > v1 then (k, v) else (k1, v1)
