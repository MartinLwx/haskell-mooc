...
union s1 s2 = Shape f
  where
    f (Coord x y) = contains s1 x y || contains s2 x y

cut :: Shape -> Shape -> Shape
cut s1 s2 = Shape f
  where
    f (Coord x y) = if contains s2 x y then False else contains s1 x y
...
paintSolid color shape (Picture b) = Picture f
  where
    f (Coord x y) = if contains shape x y then color else b (Coord x y)
...
paint (Picture f1) shape (Picture f2) = Picture f
  where
    f (Coord x y) = if contains shape x y then f1 (Coord x y) else f2 (Coord x y)
...
instance Transform Zoom where
  apply (Zoom c) (Picture f) = Picture (f . zoomCoord c)

data Flip = FlipX | FlipY | FlipXY
  deriving (Show)

instance Transform Flip where
  apply FlipX (Picture f) = Picture (f . (\(Coord x y) -> Coord (0 - x) y))
  apply FlipY (Picture f) = Picture (f . (\(Coord x y) -> Coord x (0 - y)))
...
instance Transform Blur where
  apply Blur p = Picture f
    where
      f (Coord x y) =
        blur'
          (getColor (Coord x y) p)
          (getColor (Coord (x - 1) y) p)
          (getColor (Coord (x + 1) y) p)
          (getColor (Coord x (y - 1)) p)
          (getColor (Coord x (y + 1)) p)

getColor :: Coord -> Picture -> Color
getColor coord (Picture f) = f coord

blur' :: Color -> Color -> Color -> Color -> Color -> Color
blur' (Color x y z) (Color x1 y1 z1) (Color x2 y2 z2) (Color x3 y3 z3) (Color x5 y5 z5) =
  Color
    ((x + x1 + x2 + x3 + x5) `div` 5)
    ((y + y1 + y2 + y3 + y5) `div` 5)
    ((z + z1 + z2 + z3 + z5) `div` 5)
