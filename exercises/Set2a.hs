...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = if i == 0 then [x] ++ tail xs else [head xs] ++ updateAt (i - 1) x (tail xs)
...
palindromify "" = ""
palindromify s =
  let substr = init (tail s)
   in if isPalindrome substr then substr else palindromify substr
