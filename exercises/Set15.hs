...
statements xs ys = concat $ pure (\x y -> [x ++ " is " ++ y, x ++ " is not " ++ y]) <*> xs <*> ys
...
calculator op number
  | op == "negate" = pure (0 -) <*> readMaybe number :: Maybe Int
  | op == "double" = pure (* 2) <*> readMaybe number :: Maybe Int
  | otherwise = Nothing
...
validateDiv x y = liftA2 div x' y'
  where
    x' = check True "_" x
    y' = check (y /= 0) "Division by zero!" y
...
validateAddress streetName streetNumber postCode = liftA3 Address x y z
  where
    x = check (length streetName <= 20) "Invalid street name" streetName
    y = check (all isDigit streetNumber) "Invalid street number" streetNumber
    z = check (all isDigit postCode && length postCode == 5) "Invalid postcode" postCode
...
twoPersons name1 age1 employed1 name2 age2 employed2 =
  liftA2 (\x y -> [x, y]) (pure Person <*> name1 <*> age1 <*> employed1) (pure Person <*> name2 <*> age2 <*> employed2)
...
boolOrInt s =
  let v1 = readMaybe s :: Maybe Bool
      v2 = readMaybe s :: Maybe Int
   in check (isJust v1) "Not a Bool" (Left (fromJust v1))
        <|> check (isJust v2) "Not an Int" (Right (fromJust v2))
...
normalizePhone s =
  let xs = filter (\ch -> ch /= ' ') s
   in liftA2 const (check (length xs <= 10) "Too long" xs) (allDigit xs)

allDigit xs = traverse checkDigit xs
  where
...
instance Functor Priced where
  fmap f (Priced v msg) = Priced v (f msg)

instance Applicative Priced where
  pure s = Priced 0 s
  liftA2 f (Priced v1 s1) (Priced v2 s2) = Priced (v1 + v2) (f s1 s2)
...
instance (Functor f, Functor g) => Functor (Both f g) where
  fmap fun (Both x) = Both $ fmap (fmap fun) x
